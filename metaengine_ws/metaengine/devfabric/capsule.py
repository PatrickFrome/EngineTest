...
_SECRET_PATTERNS: tuple[tuple[str, re.Pattern[bytes]], ...] = (
    ...
    ("PASSWORD_POSTGRES_URI", re.compile(rb"postgres(?:ql)?://[^:/\s]+:[^@/\s]+@", re.I)),
)
...
def _scan_secret_bytes(path: str, data: bytes) -> list[dict[str, str]]:
    hits: list[dict[str, str]] = []
    for pattern_id, pattern in _SECRET_PATTERNS:
        if pattern.search(data):
            hits.append({"path": path, "pattern": pattern_id})
    return hits
...
def build_control_capsule(root: str | Path, out: str | Path) -> dict[str, object]:
    ...
    for path in _payload_paths(root):
        ...
        secret_hits.extend(_scan_secret_bytes(rel, data))
    if secret_hits:
...
def verify_control_capsule(path: str | Path) -> dict[str, object]:
    ...
    with zipfile.ZipFile(path) as zf:
        ...
        for name in sorted(actual_names & set(expected)):
            ...
            secret_hits.extend(_scan_secret_bytes(name, data))
