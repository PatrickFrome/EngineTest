...
# I1: temperature is a mutable hyperparameter (PBT/ES can evolve it).
# Range [0.0, 2.0] — 0.0 = greedy, 2.0 = highly random. Default 0.4 (balanced).
MUTABLE_FIELDS = frozenset({"topology_id", "waves", "dialectic_operators", "max_rounds", "max_deep_engines", "exploration_rate", "temperature"})
...
class ArchitecturePolicy:
    ...
    temperature: float = 0.4  # I1: LLM sampling temperature (mutable, evolves via PBT/ES)
    ...
    def validate(self) -> None:
        ...
        # I1: temperature must be in [0.0, 2.0]
        if not 0.0 <= self.temperature <= 2.0:
            raise ValueError("POLICY_TEMPERATURE_OUT_OF_BOUNDS")
    ...
    def from_dict(cls, value: dict[str, Any]) -> "ArchitecturePolicy":
        ...
        # I1: backward-compat — inject default temperature if missing (legacy policies
        # stored before temperature was added). Then verify hash.
        if "temperature" not in value:
            value = {**value, "temperature": 0.4}
        payload = {key: item for key, item in value.items() if key != "policy_hash"}
        if claimed and canonical_hash(payload) != claimed:
            # Legacy compatibility: stored hash may have been computed WITHOUT temperature.
            # Try verifying against the legacy payload (temperature stripped).
            legacy_payload = {k: v for k, v in payload.items() if k != "temperature"}
            if canonical_hash(legacy_payload) != claimed:
        ...
        policy = cls(
            ...
            temperature=float(value.get("temperature", 0.4)),  # I1: backward-compat default
...
def mutate_policy(parent: ArchitecturePolicy, mutation_id: str, operators: tuple[str, ...], topology_id: str | None = None) -> ArchitecturePolicy:
    ...
    policy = ArchitecturePolicy(
        ...
        temperature=parent.temperature,  # I1: propagate temperature on mutation
...
class PolicyStore:
    ...
    def promote(self, candidate: ArchitecturePolicy, expected_champion_hash: str, promotion_receipt: dict[str, Any]) -> ArchitecturePolicy:
        ...
        active = ArchitecturePolicy(
            ...
            temperature=candidate.temperature,  # I1: propagate temperature on promotion
    ...
    def rollback(self, target_hash: str, reason: str) -> ArchitecturePolicy:
        ...
        active = ArchitecturePolicy(
            ...
            temperature=target.temperature,  # I1: propagate temperature on rollback
